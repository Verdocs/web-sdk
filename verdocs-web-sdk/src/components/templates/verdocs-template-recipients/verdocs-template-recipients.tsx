...
import {editRole} from '@verdocs/js-sdk/Templates/Roles';
  ...
  componentDidRender() {
    ...
    interact('.dropzone').dropzone({
      ...
      ondrop: function handleDrop(event) {
        ...
        console.log({targetSequence, targetOrder, roleName});

        const changingRole = TemplateStore.template.roles.find(role => role.name === roleName);
        console.log('changing', changingRole);
        if (changingRole) {
          ...
          editRole(this.endpoint, this.templateId, roleName, {
            name: roleName,
            template_id: this.templateId,
            type: changingRole.type,
            sequence: changingRole.sequence,
            order: changingRole.order,
          })
            .then(r => {
              console.log('Update role result', r);
            })
            .catch(e => {
              console.log('Role update failed', e);
            });
          console.log('Moved', changingRole.name, changingRole.sequence, changingRole.order);
        }
      }.bind(this),
      ondropactivate: e => {
